...
types: ["Websites", "AI"]           # 필수(≥1): 자유 멀티태그. Title Case 정규화. 필터 탭이 전체 type 합집합에서 동적 생성. (옛 단일 `category:` 파일은 빌드·콘솔에서 자동으로 types로 흡수)
...
- 카드 캡션: `**Title** – tagline` + 작은 메타줄(type 칩들 + `YEAR`). 필터바는 type 늘면 여러 줄 wrap.
...
- **롤 텍스트 링크** (`Roll.astro` + `.roll`): 호버 시 텍스트가 위로 빠지고 같은 텍스트가 아래서 올라오는 "주사위 한 칸". `::after`+`attr(data-text)`로 마크업 중복 없이. 적용처: 탑바, 필터, 푸터 소셜, **상세 백 버튼(`← All Projects`)**. 모든 내비 텍스트 링크는 이걸 쓴다(단순 색 페이드로 두지 말 것).
- **볼록 fill 버튼** (`.footer__portfolio`, 상세 `.btn`/`.btn--solid`): 호버 시 원형 캡이 아래에서 볼록하게 차오르며 글자 반전. `easeOutExpo`(`cubic-bezier(.16,1,.3,1)`)로 빠르게 올라와 끝에서 길게 멈춤. **캡은 흑↔백 모노크롬 반전만**(accent/인디고 안 씀) — 인디고는 링크 전용색. 어두운 면(`.footer__portfolio`, `.btn--solid`)은 흰 캡+검정 글자, 밝은 면(`.btn`)은 검정 캡+흰 글자.
...
**역할(중요)**: 콘솔은 **메타데이터 편집 · 순서 변경 · 삭제 전용**이다. **새 글 추가·본문 작성은 콘솔에서 안 한다** — 본문은 코드(`.md`/`.mdx`, `/showcase` 스킬·손)로만 저작하고, 콘솔은 그 frontmatter만 만진다. 덕분에 콘솔에 **본문(HTML/MDX) 저작 표면이 없어** 저장형 XSS·MDX 주입·빌드 깨짐 경로가 원천적으로 없다.

**플로우**: 로그인 → 리스트(순서 ↑↓ · Edit · Delete; `.md`+`.mdx` 모두 표시) → 에디터(폼: 메타 + types 멀티태그 + 커스텀 링크 + 썸네일 교체) → Preview(카드·헤더 메타 미리보기) → **Deploy**. 편집은 세션에 모았다가 Deploy 한 번에 **단일 커밋**(Git Data API: blob→tree→commit). 저장 시 **본문은 바이트 단위로 보존**(frontmatter만 재생성)하고 **원래 확장자(.md/.mdx)를 유지**한다. **순서**: 리스트 위치 = `order`.
...
- **백엔드**: `functions/api/*` (Pages Functions, 파일 라우팅 → `/api/*`)
  ...
  - `projects.js` (GET) 레포의 md·mdx 목록·파싱(확장자 보존, legacy `category`→`types` 폴딩) / `deploy.js` (POST) 변경셋(upserts/deletes) 원자 커밋 — frontmatter 갱신 + 본문 보존 + 썸네일 반영
